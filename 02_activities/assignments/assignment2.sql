...
WITH latest_price AS (
    SELECT
        vendor_id,
        product_id,
        cost_to_customer_per_qty AS unit_price,
        ROW_NUMBER() OVER (
...
vendor_products AS (
    ...
    LEFT JOIN latest_price lp
        ON lp.vendor_id = vi.vendor_id
       AND lp.product_id = vi.product_id
       AND lp.rn = 1
),
all_sales AS (
    SELECT
        ...
        vp.product_name,
        c.customer_id,
        5 * vp.unit_price AS sale_amount
...
VALUES (
    999,               -- unique product_id
    'Apple Pie',        -- product name
    '1 unit',           -- product size
    1,                  -- product_category_id (choose an existing or new valid category)
    'unit',             -- quantity type
...
SET current_quantity = COALESCE((
    ...
    ORDER BY vi.vendor_id DESC  -- or another column to define "last"
    LIMIT 1
), 0);
